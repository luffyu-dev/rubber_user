 ...
 */
@Slf4j
@Component(value = "userPhoneRegisterHandler")
public class UserPhoneRegisterHandler implements UserRegisterHandler{
    ...
    public UserInfoResponse register(UserRegisterInfoDto userRegisterInfoDto) {
        if (StrUtil.isEmpty(userRegisterInfoDto.getUserPhone())){
            //空号校验等
            log.error("注册的时候手机号为空");
            throw new UserRegisterException(ErrCodeEnums.PARAM_ERROR);
        }
        userRegisterInfoDto.setUserAccount(userRegisterInfoDto.getUserPhone());
